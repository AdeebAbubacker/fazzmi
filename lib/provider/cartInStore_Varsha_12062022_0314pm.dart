...
class CartCounterStore with ChangeNotifier {
  ...
  void totalPriceValue(ViewCartPageModel price) {
    ...
    for (var item in price.items!) {
      _totalPrice += (double.parse(item.price!.toString()) * item.qty!);
    }
    notifyListeners();
  }
  ...
  Future<CartResponseModel?> addProductCartLogin(
    ...
    if (_cartResponseModel!.itemId != null) {
      await setAddtoCartLoader(false);
    }
  ...
  Future<GuestCartResponsemodel?> getCartProductDetailsGuest() async {
    ...
    if (_guestCartList!.items!.isNotEmpty) {
      var data = _guestCartList!.items![0].extensionAttributes!.store;
    }
...
*/
  ...
  Future addCartFnc(
    ...
    if (quoteId != null || guestQuiteId == null) {
      if (qty > 0) {
        response = updateProductCartLogin(
            qty: qty + 1, sku: sku, productid: productid);
      } else if (qty == 0) {
        response = addProductCartLogin(qty: qty + 1, sku: sku);
      }
    }
    /* ----for guest user---- */
    else if (quoteId == null || guestQuiteId != null) {
      if (qty == 0) {
        response = addProductCartGuest(qty: qty + 1, sku: sku);
        retRes = true;
      } else if (qty > 0) {
        response = updateProductCartGuest(
            qty: qty + 1, sku: sku, productid: productid);
        retRes = true;
      }
    } else {
